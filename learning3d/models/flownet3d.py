...
from ..utils import farthest_point_sample, index_points, knn_point, pc_normalize
from ..utils import pointnet2_utils as pointutils
from ..utils import query_ball_point, square_distance
...
class PointNetSetAbstraction(nn.Module):
    ...
    def forward(self, xyz, points):
        ...
        new_points = self.queryandgroup(
            xyz_t, new_xyz.transpose(2, 1).contiguous(), points
        )  # [B, 3+C, N, S]
...
class FlowEmbedding(nn.Module):
    ...
    def forward(self, pos1, pos2, feature1, feature2):
        ...
        if self.knn:
            ...
            idx, cnt = query_ball_point(
                self.radius, self.nsample, pos2_t, pos1_t, get_cnt=True
            )
            # 利用knn取最近的那些点
    ...
    def forward(self, pos1, pos2, feature1, feature2):
        ...
        if self.knn:
            ...
            idx = query_ball_point(
                self.radius, self.nsample, pos2_t, pos1_t, get_cnt=False
            )
...
class PointNetFeaturePropogation(nn.Module):
    ...
    def forward(self, pos1, pos2, feature1, feature2):
        ...
        interpolated_feat = torch.sum(
            pointutils.grouping_operation(feature2, idx) * weight.view(B, 1, N, 3),
            dim=-1,
        )  # [B,C,N,3]
